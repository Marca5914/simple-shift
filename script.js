document.addEventListener('DOMContentLoaded', () => {
    ...
    function initialize() {
        ...
        addEventListeners(); // Make sure this is called AFTER initial renders
    }
    ...
    function renderShiftTemplates() {
        ...
        shiftTypes.forEach(shift => {
            ...
            editBtn.innerHTML = 'E'; // Use icon font/SVG for better UI
            editBtn.setAttribute('aria-label', `Edit shift type ${shift.label}`);
            editBtn.title = `Edit shift type "${shift.label}"`;
            editBtn.dataset.label = shift.label;
            editBtn.addEventListener('click', handleEditShiftType);
    ...
    // --- Handler for Clicking a Shift Template (Selection) ---
    function handleTemplateClick(event) {
        // Ignore clicks on buttons within the template
    ...
    function handleDayClick(event) {
        // Place shift only if one is selected and click wasn't on an existing shift element
    ...
    function handleDeleteShiftType(event) {
        ...
        if (confirm(`Are you sure you want to delete the shift type "${labelToDelete}"?\nThis cannot be undone and will remove it from the list of available shifts.\n(Note: Shifts already placed on the calendar using this type will NOT be removed automatically).`)) {
            ...
            if (indexToDelete > -1) {
                // Deselect if the deleted type was selected
    ...
    function handleDragStart(event) {
        // Ignore drag if started on a button
        if (event.target.classList.contains('delete-shift-type-btn') || event.target.classList.contains('edit-shift-type-btn')) {
             event.preventDefault(); return;
        }
        const shiftElement = event.target.closest('.shift-template');
        if (!shiftElement) return;
        // Deselect any selected template when starting drag
    ...
    function displayShiftOnCalendar(dayCell, shiftData) {
        ...
        // Add listener to remove the shift when clicked
        shiftDiv.addEventListener('click', (e) => {
            e.stopPropagation(); // Prevent click from bubbling to handleDayClick
            if (confirm(`Remove ${shiftData.label} shift on ${dayCell.dataset.date}?`)) {
                 ...
                 shiftDiv.remove(); // Remove from DOM
            }
        });
    ...
    function handleSaveShiftType(event) {
        ...
        const label = shiftLabelInput.value.trim(); // Read label even if disabled (for finding item)
    ...
    function addEventListeners() {
        ...
        addShiftForm.addEventListener('submit', handleSaveShiftType); // Use the combined save function
